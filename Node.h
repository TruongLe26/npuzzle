//
// Created by DELL 7480 on 11/20/2023.
//
...
public:
    ...
    int Heuristic(const Node &goal) const {
        ...
        for (int i = 0; i < boardSqSize; ++i) {
            for (int j = 0; j < boardSqSize; ++j) {
                if (A[i][j] != 0) {
                    int value = A[i][j] - 1; // Subtract 1 for 0-indexed array
                    int targetRow = value / boardSqSize;
                    int targetCol = value % boardSqSize;
